...
def test(params, test_file):
    # load model
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    if params.model == 'Transformer':
        model = TransformerModel(n_way=params.n_way, n_feature=200, n_head=8,
    ...
    # initialize dataset
    test_data = TestSet(params.n_way, params.k_shot, params.k_query, test_file)
    ...
    for epoch in range(400):
        ...
        logits = model(x_spt)
        loss = criterion(logits, y_spt.long())
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        if epoch % 40 == 0:
            ...
            with torch.no_grad():
                ...
                # scalar
                correct = torch.eq(pred_q, y_qry).sum().item() / qrysz * 100
                ...
                print("[Epoch %d]\tTrain loss: %.3f, Test acc: %.3f+-%.3f" % (epoch, loss.data, correct, std.data))
...
if __name__ == '__main__':
    ...
    if params.k_query == -1:
        params.k_query = 300 - params.k_shot

    print("--------------------------Env1--------------------------")
    # params.n_way = 3
    # acc, std = test(params, "env1_testing.npy")
    # print("Unseen %.2f +- %.2f" % (acc, std.numpy()))

    print("--------------------------Env2--------------------------")
    if params.k_query == 300 - params.k_shot:
        params.k_query = 60 - params.k_shot

    # params.n_way = 3
    # acc, std = test(params, "env2_testing.npy")
    # print("Unseen %.2f +- %.2f" % (acc, std.numpy()))

    params.n_way = 8
    acc, std = test(params, "env2_testing.npy")
    print("All %.2f +- %.2f" % (acc, std.numpy()))

    print("--------------------------Env3--------------------------")
    # params.n_way = 3
    # acc, std = test(params, "env3_testing.npy")
    # print("Unseen %.2f +- %.2f" % (acc, std.numpy()))
    ...
    print("--------------------------Env4--------------------------")
    # params.n_way = 3
    # acc, std = test(params, "env4_testing.npy")
    # print("Unseen %.2f +- %.2f" % (acc, std.numpy()))
