...
from data.datamgr import SimpleFewShotDataManager, SetDataManager, SetNpyManager, NpyDataManager
from methods.baselinetrain import BaselineTrain
from methods.protonet import ProtoNet
from methods.matchingnet import MatchingNet
from methods.relationnet import RelationNet
from methods.gnnnet import GnnNet
from methods.gnnnet_isda import GnnNet_ISDA
...
def train(base_loader, val_loader, model, start_epoch, stop_epoch, params):
    ...
    max_acc = 0
    total_it = 0
    model_name = model._get_name().split("_")

    # start
    for epoch in range(start_epoch,stop_epoch):
        model.train()
        # print(model_name)
        if ISDA in model_name or isda in model_name:
            total_it = model.train_loop(epoch, base_loader, optimizer, total_it, params.lambda_0)
        else:
...
if __name__=='__main__':
    ...
    if params.dataset == 'multi':
        ...
        datasets = ['miniImagenet', "radar", "env2", 'env3', 'env4']
    ...
    if params.method in ['baseline', 'baseline++'] :
        ...
        base_datamgr    = NpyDataManager(image_size, batch_size=16)
        base_loader     = base_datamgr.get_data_loader( base_file , aug=params.train_aug )
        val_datamgr     = NpyDataManager(image_size, batch_size=64)
    ...
    elif params.method in ['protonet','matchingnet','relationnet', 'relationnet_softmax', 'gnnnet', 'gnnnet_isda']:
        ...
        base_datamgr            = SetNpyManager(image_size, n_query = n_query,  **train_few_shot_params)
        base_loader             = base_datamgr.get_data_loader( base_file , aug = params.train_aug, seq=params.model.endswith('Vit'))

        test_few_shot_params     = dict(n_way = params.test_n_way, n_support = params.n_shot)
        val_datamgr             = SetNpyManager(image_size, n_query = n_query, **test_few_shot_params)
        val_loader              = val_datamgr.get_data_loader( val_file, aug = False, seq=params.model.endswith('Vit'))
        ...
        elif params.method == "gnnnet_isda":
            model           = GnnNet_ISDA(model_dict[params.model], tf_path=params.tf_dir, **train_few_shot_params)
    ...
    elif 'baseline' not in params.method and 'Vit' not in params.model:
