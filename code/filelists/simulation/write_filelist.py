...
data_dir = "/home/yangmuzhen/NAHAR/data/simulation_10s_by_people/"
...
if __name__ == '__main__':
    ...
    for i, action in enumerate(seen_actions):
        ...
        seen_test_data = seen_test_data + ["%s %d" % (file, i) for file in files[:60]]

    for i, action in enumerate(unseen_actions):
        files = sorted(glob.glob(data_dir + action + "/*.npy"))
        length = len(files)
        val_data = val_data + ["%s %d" % (file, 5 + i) for file in files[60:]]
        unseen_test_data = unseen_test_data + ["%s %d" % (file, 5 + i) for file in files[:60]]
