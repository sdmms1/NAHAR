...
class Gconv_without_fc(nn.Module):
  maml = False
  def __init__(self, nf_input, nf_output, J):
    super(Gconv_without_fc, self).__init__()
    self.J = J
...
class GNN_nl(nn.Module):
  def __init__(self, input_features, nf, train_N_way):
    ...
    self.layer_last = Gconv_without_fc(self.input_features + int(self.nf / 2) * self.num_layers, train_N_way, 2)
    self.fc = nn.Linear(2 * (self.input_features + int(self.nf / 2) * self.num_layers), train_N_way)
  ...
  def forward(self, x):
    ...
    out_size, out_ = self.layer_last([Wl, x])
    out_temp = out_.view(-1, 2 * (self.input_features + int(self.nf / 2) * self.num_layers))
    out = self.fc(out_temp)
