...
class Efficient_Net(nn.Module):
  def __init__(self, name, in_channels, flatten = True, leakyrelu = False):
    super(Efficient_Net, self).__init__()
    self.final_feat_dim = 1200
    self.model = EfficientNet.from_name(model_name = name , in_channels = in_channels, num_classes=self.final_feat_dim )
  def forward(self, x):
    return self.model(x)
