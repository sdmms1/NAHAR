...
class MetaISDATemplate(nn.Module):
  ...
  @abstractmethod
  def set_forward_loss(self, x, ratio, isda):
    pass
  ...
  def correct(self, x):
    scores, loss = self.set_forward_loss(x, 0, False)
  ...
  def train_loop(self, epoch, train_loader, optimizer, total_it, lambda_0):
    ...
    for i, (x,_ ) in enumerate(train_loader):
      ...
      if self.change_way:
        self.n_way  = x.size(0)
      optimizer.zero_grad()
      ratio = lambda_0 * (epoch * len(train_loader) + (i + 1)) / (total_it + 1) 
      _, loss = self.set_forward_loss(x, ratio, True)
      loss.backward()
      optimizer.step()
