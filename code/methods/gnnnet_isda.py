...
import numpy as np
from .meta_isda_template import MetaISDATemplate
from .gnn_isda import GNN_nl
from . import backbone
from .ISDA import ISDALoss

class GnnNet_ISDA(MetaISDATemplate):
  maml=False
  def __init__(self, model_func,  n_way, n_support, tf_path=None):
    super(GnnNet_ISDA, self).__init__(model_func, n_way, n_support, tf_path=tf_path)

    # loss function
    self.loss_fn = ISDALoss(454, self.n_way)
    self.second_loss_fn = nn.CrossEntropyLoss()
    # metric function
  ...
  def set_forward(self,x,is_feature=False, isda = False):
    x = x.cuda()
    if is_feature:
    ...
    features, scores = self.forward_gnn(z_stack)
    if isda:
  ...
  def forward_gnn(self, zs):
    ...
    features, scores = self.gnn(nodes)
    ...
    features = features.view(self.n_query, self.n_way, self.n_support + 1, feature_size[-1])[:,:, -1].permute(1, 0, 2).reshape(-1, feature_size[-1])
    return features, scores


  def set_forward_loss(self, x, ratio, isda = True):
    y_query = torch.from_numpy(np.repeat(range( self.n_way ), self.n_query))
    y_query = y_query.cuda()
    if isda:
      z, scores = self.set_forward(x, isda = True)
    # z = torch.reshape(z, (-1, z.shape[-1]))
      loss = self.loss_fn(self.gnn.fc, scores, z, y_query, ratio)
    else:
      scores = self.set_forward(x, isda = False)
      loss = self.second_loss_fn( scores , y_query)
