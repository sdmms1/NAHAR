...
if __name__=='__main__':
    ...
    for model_name in ["simulation"]:
        ...
        for single in [True, False]:
            ...
            testset1, testset2 = 'simulation', 'radar'
            for same_people in [True, False]:
                for test_name in ["seen_test", "unseen_test", "test"]:
                    test_datamgr = CrossFewShotDataManager("%s/%s/%s.txt" % (params.data_dir, testset1, test_name),
                                                           "%s/%s/%s.txt" % (params.data_dir, testset2, test_name))
                    print("--------%s %s %s--------" % (test_name, 'same' if same_people else 'different',
                                                        'single' if single else 'multiple'))
                    test_loader = test_datamgr.get_data_loader(few_shot_params, tep=tep,
